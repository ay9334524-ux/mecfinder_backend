...
const getReviewsByMechanic = asyncHandler(async (req, res) => {
  ...
  const stats = await Review.aggregate([
    { $match: { mechanicId: new (require('mongoose')).Types.ObjectId(mechanicId), status: 'APPROVED' } },
    {
      $group: {
        _id: null,
...
const getReviewsByService = asyncHandler(async (req, res) => {
  ...
  const stats = await Review.aggregate([
    { $match: { serviceId: new (require('mongoose')).Types.ObjectId(serviceId), status: 'APPROVED' } },
    {
      $group: {
        _id: null,
        averageRating: { $avg: '$rating' },
...
const getReviewStatistics = asyncHandler(async (req, res) => {
  ...
  if (mechanicId) matchStage.mechanicId = new (require('mongoose')).Types.ObjectId(mechanicId);
  if (serviceId) matchStage.serviceId = new (require('mongoose')).Types.ObjectId(serviceId);
