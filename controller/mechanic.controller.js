...
const toggleOnline = asyncHandler(async (req, res) => {
  ...
  // When going online, auto-release stale isBusy state if the associated booking
  // is already completed/cancelled (prevents mechanics being stuck in busy state).
  if (mechanic.isOnline && mechanic.isBusy && mechanic.currentBookingId) {
    const Booking = require('../models/Booking');
    const staleBooking = await Booking.findById(mechanic.currentBookingId)
      .select('status paymentStatus').lean();
    const staleStatuses = ['COMPLETED', 'CANCELLED', 'NO_MECHANIC_AVAILABLE'];
    if (!staleBooking || staleStatuses.includes(staleBooking.status)) {
      mechanic.isBusy = false;
      mechanic.currentBookingId = null;
      console.log(`🔓 Auto-released stale isBusy for mechanic ${mechanic._id}`);
    }
  }
...
/**
 * Update payout UPI ID only (merge — does not wipe bank fields).
 * PUT /api/mechanic/bank-details/upi
 */
const updatePayoutUpi = asyncHandler(async (req, res) => {
  const { upiId } = req.body;
  const trimmed = String(upiId || '').trim();
  if (!/^[\w.-]+@[\w]+$/.test(trimmed)) {
    return ApiResponse.badRequest(res, 'Invalid UPI ID format (e.g. name@paytm)');
  }

  const mechanic = await Mechanic.findByIdAndUpdate(
    req.mechanic.id,
    { $set: { 'bankDetails.upiId': trimmed } },
    { new: true, runValidators: true },
  ).select('bankDetails');
  ...
  const masked = mechanic.bankDetails
    ? {
        ...mechanic.bankDetails.toObject(),
        accountNumber: mechanic.bankDetails.accountNumber
          ? `****${String(mechanic.bankDetails.accountNumber).slice(-4)}`
          : null,
      }
    : null;

  ApiResponse.success(res, { bankDetails: masked }, 'UPI ID saved for payouts');
});
