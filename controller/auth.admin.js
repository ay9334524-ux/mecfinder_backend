...
const refreshToken = async (req, res) => {
  try {
    ...
    // Verify refresh token (rotation-aware: accepts JWT_SECRET_PREVIOUS too)
    let decoded;
    try {
      decoded = tokenService.verifyAccessJwt(token);
    } catch (err) {
...
/**
 * Logout admin — revokes the current access token via Redis blocklist so it
 * cannot be reused even if the client retains a copy.
 */
  ...
  try {
    if (req.adminAccessToken) {
      try {
        ...
        const tokenHash = crypto.createHash('sha256').update(req.adminAccessToken).digest('hex');
        const decoded = tokenService.verifyAccessJwt(req.adminAccessToken, { ignoreExpiration: true });
