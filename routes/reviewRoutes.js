...
const { validate, reviewValidations } = require('../utils/validation');

const router = express.Router();

// User routes (authenticated users can submit reviews)
router.post('/create', authMiddleware, validate(reviewValidations.create), createReview);
...
router.patch('/admin/:reviewId/reject', authMiddleware, requireRole('ADMIN', 'SUPER_ADMIN'), validate(reviewValidations.reject), rejectReview);
router.patch('/admin/:reviewId/flag', authMiddleware, requireRole('ADMIN', 'SUPER_ADMIN'), validate(reviewValidations.flag), flagReview);
router.patch('/admin/:reviewId/respond', authMiddleware, requireRole('ADMIN', 'SUPER_ADMIN'), validate(reviewValidations.respond), respondToReview);
