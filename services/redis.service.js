...
class RedisService {
  ...
  async setMechanicOnline(mechanicId, locationData, expirySeconds = 3600) {
    const key = `mechanic:online:${mechanicId}`;
    await this.set(key, { ...locationData, lastSeen: Date.now() }, expirySeconds);
  }

  /** Extend presence TTL without clobbering JSON payload (used on socket location pings). */
  async refreshMechanicOnlineTtl(mechanicId, ttlSeconds = 3600) {
    ...
    try {
      ...
      if (exists) {
        ...
        // No key yet (e.g. socket-only) — minimal presence so multi-node reachability works.
        await this.set(key, { lastSeen: Date.now(), source: 'socket' }, ttlSeconds);
      }
    } catch (_) {
      /* ignore */
    }
  ...
  // ──────────────────────────────────────────────────────────────────────
  // JWT blocklist — revoke access tokens before they expire (logout, etc.)
  // We hash the token so the raw secret material is never stored in Redis.
  // ──────────────────────────────────────────────────────────────────────
  async blocklistToken(tokenHash, ttlSeconds) {
    if (!this.client || !this.isConnected) return false;
    const safeTtl = Math.max(1, Math.floor(ttlSeconds));
    await this.client.setEx(`jwt:blocklist:${tokenHash}`, safeTtl, '1');
    return true;
  }

  async isTokenBlocklisted(tokenHash) {
    if (!this.client || !this.isConnected) return false;
    const exists = await this.client.exists(`jwt:blocklist:${tokenHash}`);
    return exists === 1;
  }
