...
const authenticateToken = async (req, res, next) => {
  try {
    ...
    // Check blocklist (set on logout). Best-effort: if Redis is unavailable
    // we fail open (just like before) so the API stays usable.
    try {
      const tokenHash = tokenService.hashToken(token);
      const revoked = await redisService.isTokenBlocklisted(tokenHash);
      if (revoked) {
        return res.status(401).json({ success: false, message: 'Token revoked', code: 'TOKEN_REVOKED' });
      }
    } catch (_) { /* Redis offline — allow */ }

    req.user = result.decoded;
    // Expose raw token + decoded for downstream blocklist (e.g. logout).
...
const requireRole = (...roles) => {
  return (req, res, next) => {
    ...
    if (req.user.role === 'MECHANIC') {
      req.mechanic = {
        ...req.user,
        id: tokenService.normalizeUserId(req.user.id),
      };
    }
